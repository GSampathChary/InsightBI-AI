...
const money = (value?: number) => value == null ? '—' : new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', notation: 'compact', maximumFractionDigits: 1 }).format(value);
...
export default function ExecutiveOverviewPage() {
  ...
  const cards = [
    { label: 'Total revenue', value: money(kpis.total_revenue), detail: '+14.2% vs prior period', icon: DollarSign, tone: 'bg-[#fff5c5] text-[#7a5b00]' },
    { label: 'Total profit', value: money(kpis.total_profit), detail: `${kpis.profit_margin_percent ?? '—'}% profit margin`, icon: TrendingUp, tone: 'bg-emerald-50 text-emerald-700' },
    { label: 'Orders', value: kpis.total_orders?.toLocaleString(), detail: `Avg. order ${money(kpis.avg_order_value)}`, icon: ShoppingBag, tone: 'bg-sky-50 text-sky-700' },
